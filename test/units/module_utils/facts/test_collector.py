...
class TestCollectorClassesFromGatherSubset(unittest.TestCase):
    ...
    def test_smart_gathering_with_different_subsets(self):
        """Test that smart gathering with different gather_subsets across plays properly handles fact dependencies.
        This simulates the scenario where:
        - Play 1 uses gather_subset: !all (minimal facts)
        - Play 2 uses gather_subset: hardware
        - Fact caching is enabled
        - gather_facts is set to 'smart'
        """
        # First play with !all should only collect minimal facts
        first_play_res = self._classes(
            all_collector_classes=default_collectors.collectors,
            gather_subset=['!all'],
            minimal_gather_subset=frozenset(['platform']),  # Simulate minimal facts
            gather_timeout=10
        )

        # Verify first play only collected minimal facts
        self.assertNotIn(default_collectors.LinuxHardwareCollector, first_play_res)
        self.assertIn(default_collectors.PlatformFactCollector, first_play_res)

        # Second play requesting hardware facts
        # In reality, this would use cached facts from first play
        second_play_res = self._classes(
            all_collector_classes=default_collectors.collectors,
            gather_subset=['hardware'],
        ...
        # Verify second play would normally collect hardware facts
        self.assertIn(default_collectors.LinuxHardwareCollector, second_play_res)

        # This demonstrates the potential issue:
        # - First play cached facts don't include hardware
        # - Second play would expect hardware facts to be available
        # - But smart gathering would skip collection since facts are cached
        self.assertEqual(set(first_play_res), set(second_play_res))
        
        # Verify hardware facts collector is in the difference between plays
        facts_diff = set(second_play_res) - set(first_play_res)
        self.assertIn(default_collectors.LinuxHardwareCollector, facts_diff)
